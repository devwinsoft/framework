...
namespace Devarc
{
    public class Compiller
    {
        ...
        {
            string bakupCurDir = Directory.GetCurrentDirectory();
            ...
            if (System.Diagnostics.Debugger.IsAttached)
                ...
                appDir = Path.GetDirectoryName(System.Reflection.Assembly.GetEntryAssembly().Location);
            ...
            {
                Console.WriteLine("Cannot find config file: {0}", _cfgPath);
                return;
            }

            // Read Config
            StreamReader cfgReader = File.OpenText(_cfgPath);
            string cfgData = cfgReader.ReadToEnd();
            cfgReader.Close();

            JsonData jsonRoot = JsonMapper.ToObject(cfgData);
            string srcFile = jsonRoot["SOURCE"].ToString();
            JsonData dataRoot = jsonRoot["DATAS"];
            ...
            for (int i = 0; i < dataFiles.Length; i++)
            {
                ...
                if (tmpPath.Length > 2 && tmpPath[1] == ':')
                    ...
                    dataFiles[i] = Path.Combine(appDir, tmpPath);
            }

            if (MakeDLL(dataFiles) == false)
            {
                Console.WriteLine("Cannot compile data files.");
                Directory.SetCurrentDirectory(bakupCurDir);
                return;
            }
            ...
            if (srcFile.Length > 2 && srcFile[1] == ':')
                ...
                srcFullPath = Path.Combine(bakupCurDir, srcFile);
            ...
            // Read Source
            StreamReader srcReader = File.OpenText(srcFullPath);
            string srcData = srcReader.ReadToEnd();
            srcReader.Close();
        ...
        {
            string outputPath = Path.Combine(Directory.GetCurrentDirectory(), "temp.dll");
            ...
            {
                CSharpCodeProvider provider = new CSharpCodeProvider();
                string dllSource = builder.ToString();
                var cr = provider.CompileAssemblyFromSource(cp, dllSource);
                return true;
            }
            catch (Exception ex)
            {
