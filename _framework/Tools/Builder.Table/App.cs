    ...
    class App
    {
        static void Main(string[] args)
        {
            ...
            if (args.Length > 1)
            {
                ...
                {
                    if (args.Length > 2)
                        builder1.Export(args[1], args[2]);
                    else
                        builder1.Export(args[1], Path.GetDirectoryName(args[1]));
                }
                else if (args.Contains<string>("-data"))
                {
                    if (args.Length > 2)
                        builder2.Export(args[1], args[2]);
                    else
                        builder2.Export(args[1], Path.GetDirectoryName(args[1]));
                }
            }
            else
            {
