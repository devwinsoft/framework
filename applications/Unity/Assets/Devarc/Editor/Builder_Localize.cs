...
class Builder_Localize : Builder_Base
{
    ...
    public bool BuildDataFile(DATA_FILE_TYPE _saveFileType, string[] _inFileList, string[] _outDirList)
    {
        ...
        if (assem != null)
        {
            ...
            {
                string tmpFilePath = Path.GetFullPath(Path.Combine(Application.dataPath, tmpFile));
                string tmpTableName = GetClassNameEx(tmpFile);
    ...
    {
        string className = GetClassName(sheet_name);
