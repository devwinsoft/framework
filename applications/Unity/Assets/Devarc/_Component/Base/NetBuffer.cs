...
namespace Devarc
{
    public class NetBuffer
    {
        ...
        public NetBuffer(byte[] _buf)
        {
            stream = new MemoryStream(_buf);
        }
        ...
        public int ReadBytes(int cnt, out byte[] data)
        {
            data = new byte[cnt];
            return stream.Read(data, 0, cnt);
        }

        public float ReadFloat()
        {
            int len = stream.Read(mBuf, 0, sizeof(float));
            return BitConverter.ToSingle(mBuf, len - sizeof(float));
        }

        public Int16 ReadInt16()
        {
            int len = stream.Read(mBuf, 0, sizeof(Int16));
            return BitConverter.ToInt16(mBuf, len - sizeof(Int16));
        }

        public Int32 ReadInt32()
        {
            int len = stream.Read(mBuf, 0, sizeof(Int32));
            return BitConverter.ToInt32(mBuf, len - sizeof(Int32));
        }

        public UInt32 ReadUInt32()
        {
            int len = stream.Read(mBuf, 0, sizeof(UInt32));
            return BitConverter.ToUInt32(mBuf, len - sizeof(UInt32));
        }

        public Int64 ReadInt64()
        {
            int len = stream.Read(mBuf, 0, sizeof(Int64));
            return BitConverter.ToInt64(mBuf, len - sizeof(Int64));
        }

        public string ReadString()
        {
            int data_len = ReadInt16();
            int read_len = stream.Read(mBuf, 0, data_len);
            return encoding.GetString(mBuf, 0, read_len);
        }
