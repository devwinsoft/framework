...
{
    [System.Serializable]
    public class FString : IBaseObejct<string>
    {
        public string Key = "";
        public string Value = "";
    ...
    public class T_FString : FString, IBaseObejct<string>, IDisposable
    {
        public static Container<T_FString, string> MAP = new Container<T_FString, string>();
        public static T_FString Get(string _key)
        {
            SqliteCommand cmd = new SqliteCommand(TableManager.SQLite_Connection);
            cmd.CommandText = string.Format("select Key, Value from string where string='{1}';", _key);
            SqliteDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
            {
                T_FString obj = new T_FString();
